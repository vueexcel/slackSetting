<template>
  <div>
    ...
    <i class="fa fa-circle" :style="{'color': '#C0C0C0'}" />
    <span>&nbsp;Default color</span>
    <b-row>
      <b-col cols="12" sm="6" lg="4">
        <b-form-select @change="getSlackMessage()" v-model="selected" :options="options"></b-form-select>
      </b-col>
    </b-row>
    <div v-if="allMessage.length">
      <b-row class="justify-content-md-center" v-for="(msg , key) in allMessage" :key="key">
        <b-col>
          ...
          <div
            class="pl-1 mb-5 bg-white mt-3 pt-2"
            :style="[msg.message_color ? {'borderLeft': '4px solid ' + msg.message_color} : {'borderLeft': '4px solid #C0C0C0'}]"
          >
            <span v-if="!msg.edit" class="font-weight-bold px-2">
              {{ msg.message }}
              <b-badge variant="primary" class="ml-3" pill>{{msg.message_type}}</b-badge>
            </span>
            <span v-if="msg.edit">
              <input
                class="form-control"
    ...
    <div v-else class="w-100 text-center" >
      <i class="fa fa-circle-o-notch fa-spin fa-2x m-auto"></i>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    api_getSlackSettings: call("notificationMessage/slackSettings"),
    api_setSlackSettings: call("notificationMessage/setSlackSettings"),
    // api_getSchedularMsg: call("notificationMessage/schedularMsg"),
    // api_setSchedularMsg: call("notificationMessage/setSchedularMsg"),
    api_slackMessage: call("notificationMessage/slackMessage"),
    api_getSlackChannels: call("notificationMessage/getSlackChannels"),
    api_updateSlackChannels: call("notificationMessage/updateSlackChannels"),
    getSlackSettings() {
    ...
    async getSlackMessage() {
      this.allMessage = [];
      const platform = this.selected || "HR";
      this.api_slackMessage(platform)
        .then(res => {
